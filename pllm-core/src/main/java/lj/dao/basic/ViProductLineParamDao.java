...
public class ViProductLineParamDao extends JdbcDao<ViProductLineParam> implements IViProductLineParamDao {
	...
	public long insert(ViProductLineParam obj){
	    String sql="insert into ViProductLineParam(productLineId,paramType,paramUp,paramDown,productName,productVersion) ";
		sql=sql+" values(?,?,?,?,?,?)";
		int intRet=this.jdbcTemplate.update(sql,obj.getProductLineId(),obj.getParamType(),obj.getParamUp(),obj.getParamDown(),obj.getProductName(),obj.getProductVersion());
		if(intRet<=0)
	...
	public String update(ViProductLineParam obj){
		...
		sql=sql+" productLineId=?,paramType=?,paramUp=?,paramDown=?,productName=?,productVersion=?";
		sql=sql+" where productLineParamId=?";
		int intRet = this.jdbcTemplate.update(sql
		,obj.getProductLineId(),obj.getParamType(),obj.getParamUp(),obj.getParamDown(),obj.getProductName(),obj.getProductVersion(),obj.getProductLineParamId());
		if (intRet <= 0)
     ...
     */
	@Override
	public Pager<ViProductLineParam> findAllPaged(String queryProductLineId,
    		String queryParamName,String queryParamType){
		String sql="select * from ViProductLineParam";
		Map<String,Object> params=new HashMap<String,Object>();
		params.put("productLineId", queryProductLineId);
		params.put("paramName", queryParamName);
		params.put("paramType", queryParamType);
