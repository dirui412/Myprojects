...
<div class="row">
	<div class="col-xs-12">
		<form class="form-horizontal" role="form">
		    <input type="hidden" name="queryUserProductLineId" id="queryUserProductLineId" value="${userProductLineId}" />
			...
			<div style="text-align: right; float: right;">
				...
				<button id="btnReturn" class="btn btn-sm btn-blue">返回</button>
			</div>
		</form>
	</div>
</div>
			...
			<div class="modal-body">
				<div class="row">
					<form id="formInput" class="form-horizontal" role="form" >
						<input type="hidden" id="hiddenUserEquipmentId" name="hiddenUserEquipmentId" />
						<input type="hidden" name="inputUserProductLineId" id="inputUserProductLineId" value="${userProductLineId}" />
						<div class="form-group">
							<label class="col-sm-2 control-label no-padding-right"
								for="inputEquipmentId">生产线设备</label>
							<div class="col-sm-4">
								<select class="chosen-select form-control" id="inputEquipmentId"
									name="inputEquipmentId" required>
									<option value=""></option>
									<c:forEach var="item" items="${productLineEquipments}">
...
<!-- 设备安装窗口 -->
<div id="divInstallForm" class="modal" tabindex="-1">
	<div class="modal-dialog" style="width: 800px; height: 600px">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal">&times;</button>
			...
			<div class="modal-body">
				<div class="row">
					<form id="formInstall" class="form-horizontal" role="form" >
						<div class="form-group">
							<label class="col-sm-2 control-label no-padding-right"
								for="inputInstallTime">安装时间</label>
							<div class="col-sm-4">
								<input type="text" id="inputInstallTime" name="inputInstallTime" size="16" readonly required /> <span class="addon">
									<i class="fa fa-clock-o"></i>
								</span>
							</div>
							<label class="col-sm-2 control-label no-padding-right"
								for="inputInstallParams">安装参数</label>
							<div class="col-sm-4">
								<input type="text" id="inputInstallParams" name="inputInstallParams"  required />
							</div>
						</div>
						<div class="form-group">
							<label class="col-sm-2 control-label no-padding-right"
								for="inputInstallMemo">按照说明</label>
							<div class="col-sm-4">
								<input type="text" id="inputInstallMemo" name="inputInstallMemo"  required />
							</div>
						</div>
					</form>
				</div>
			...
			<div class="modal-footer">
				<button id="btnCancelInstall" class="btn btn-sm" data-dismiss="modal">
				    <i class="ace-icon fa fa-times"></i> 取消
				</button>
				<button id="btnOkInstall" class="btn btn-success btn-sm">
				    <i class="ace-icon fa fa-check"></i> 保存
				</button>
			</div>
		</div>
...
<script type="text/javascript">
													...
													},
													{
														...
														"render" : function(
															...
															var str = '<div class="center" >';
														    	...
														    	str = str
																	+ '<a href="javascript:inputInstallInfo('
																	+ meta.row
																	+ ')" >设备安装</a>';
															str = str
																	+ '</div>';
	...
	jQuery(function($) {
		...
		$('#btnAdd').on('click', function(e) {
			$("#hiddenUserEquipmentId").val(-1);
		    $("#inputEquipmentId").val("");
			$('#divInputForm').modal();
			e.preventDefault();
		});
		//保存信息
		...
		//保存安装信息
		$('#btnOkInstall').on('click', function(e) {
			//2-窗口验证,成功提交数据
			if($("#formInstall").valid()==true){
				installEquipment();
				e.preventDefault(); //阻止默认事件Post
			}
		});
	...
	function editObject(rowIndex) {
		...
		$("#hiddenUserEquipmentId").val(row["userEquipmentId"]);
		$("#inputEquipmentId").val(row["equipmentId"]);
		$('#divInputForm').modal();
	}
	...
	function postObject() {
	    var obj=new Object();
		obj.userEquipmentId =$("#hiddenUserEquipmentId").val();
		obj.userProductLineId=$("#inputUserProductLineId").val();
		obj.equipmentId=$("#inputEquipmentId").val();
		var jsonStr = JSON.stringify(obj);
		//alert(jsonStr);
		var httpMethod="post";
		if($("#hiddenUserEquipmentId").val()<=0)
		  httpMethod="post";
		else
		  httpMethod="put";
		$.ajax({
	...
	function deleteObject() {
		...
		$.ajax({
			type : "delete",
			url : "../install/viUserEquipment/"+deleteId,
			async : false, //默认为true 异步   
			error : function() {
				alert('提交失败!');
			},
	...
	$('#inputInstallTime').datetimepicker({
		language : 'zh',
	...
	//输入安装信息
	function inputInstallInfo(rowIndex)
	{	
		if (dataTableObj.data().length <= 0)
			return;
		if (rowIndex < 0)
		...
		var row = dataTableObj.data()[rowIndex];
		$("#hiddenUserEquipmentId").val(row["userEquipmentId"]);
		//alert($("#hiddenUserEquipmentId").val());
	    $("#inputEquipmentId").val(row["equipmentId"]);
	    $("#inputInstallTime").val(row["installTime"]);
	    $("#inputInstallParams").val(row["installParams"]);
	    $("#inputInstallMemo").val(row["installMemo"]);
		$('#divInstallForm').modal();
	}
	...
	function installEquipment() {
		...
		obj.userProductLineId=$("#inputUserProductLineId").val();
		obj.equipmentId=$("#inputEquipmentId").val();
		obj.installTime=$("#inputInstallTime").val();
		obj.installParams=$("#inputInstallParams").val();
		obj.installMemo=$("#inputInstallMemo").val();
		var jsonStr = JSON.stringify(obj);
		//alert(jsonStr);
		var httpMethod="post";
		if($("#hiddenUserEquipmentId").val()<=0)
