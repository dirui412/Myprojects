...
<div class="row">
	<div class="col-xs-12">
		<form class="form-horizontal" role="form">
		    <input type="hidden" name="queryGroupId" id="queryGroupId" value="${groupId}" />
					...
					<form id="formInput" class="form-horizontal" role="form" >
                        ...
                        <div class="form-group">
							<label class="col-sm-2 control-label no-padding-right"
								for="inputGroupName">客户厂商</label>
							<div class="col-sm-4">
								<input type="text" id="inputGroupName" name="inputGroupName"  readonly />
							</div>
							<label class="col-sm-2 control-label no-padding-right"
								for="inputProductName">生产线名称</label>
							<div class="col-sm-4">
								<input type="text" id="inputProductName" name="inputProductName"  readonly />
							</div>
						</div>
						<div class="form-group">
							<label class="col-sm-2 control-label no-padding-right"
								for="inputLectotypeUserName">选型用户</label>
							<div class="col-sm-4">
								<input type="text" id="inputLectotypeUserName" name="inputLectotypeUserName"  readonly />
							</div>
							<label class="col-sm-2 control-label no-padding-right"
								for="inputLectotypeTime">选型时间</label>
							<div class="col-sm-4">
								<input type="text" id="inputLectotypeTime" name="inputLectotypeTime"  readonly />
							</div>
						</div>
						<div class="form-group">
							<label class="col-sm-2 control-label no-padding-right"
							...
							<div class="col-sm-4">
								<input type="text" id="inputLectotypeParams" name="inputLectotypeParams"  readonly />
							</div>
							<label class="col-sm-2 control-label no-padding-right"
								for="inputLectotypeMemo">选型说明</label>
							<div class="col-sm-4">
								<input type="text" id="inputLectotypeMemo" name="inputLectotypeMemo"  readonly />
							</div>
						</div>
					
						<div class="form-group">
							<label class="col-sm-2 control-label no-padding-right"
								for="inputCheckTime">验收时间</label>
							<div class="col-sm-4">
								<input type="text" id="inputCheckTime" name="inputCheckTime"  size="16" type="text" readonly required /> <span class="addon">
									<i class="fa fa-clock-o"></i>
								</span>
							</div>
							<label class="col-sm-2 control-label no-padding-right"
								for="inputCheckMemo">验收说明</label>
							<div class="col-sm-4">
								<input type="text" id="inputCheckMemo" name="inputCheckMemo"  required />
							</div>
						</div>
					</form>
				</div>
...
<script type="text/javascript">
								...
								.DataTable(
										{
											...
											"ajax" : {
												url : '../install/viUserProductLineCheck/',
												type : 'get',
												data : function(d) {
	...
	function editObject(rowIndex) {
		...
		$("#hiddenCheckId").val(row["checkId"]);


		$("#inputProductLineState").val(row["productLineState"]);
		...
		$("#inputLectotypeUserName").val(row["lectotypeUserName"]);
		$("#inputProductName").val(row["productName"]);
		$("#inputProductType").val(row["productType"]);
		$("#inputCheckTime").val(row["checkTime"]);
		$("#inputCheckMemo").val(row["checkMemo"]);
		$('#divInputForm').modal();
	}
	...
	function postObject() {
		...
		obj.productLineState=$("#inputProductLineState").val();
		obj.lectotypeUserId=$("#inputLectotypeUserId").val();
		...
		obj.lectotypeUserName=$("#inputLectotypeUserName").val();
		obj.productName=$("#inputProductName").val();
		obj.productType=$("#inputProductType").val();
		obj.checkId=$("#inputCheckId").val();

		obj.checkTime=$("#inputCheckTime").val();
		obj.checkMemo=$("#inputCheckMemo").val();
		...
		$.ajax({
			type : httpMethod,
			url : "../install/checkUserProductLine/",
			data : jsonStr,
