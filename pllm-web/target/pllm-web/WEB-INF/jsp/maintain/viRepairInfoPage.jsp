			...
			<div class="modal-body">
				<div class="row">
					<form id="formInput" class="form-horizontal" role="form" >
						<input type="hidden" id="hiddenRepairId" name="hiddenRepairId" />
						<div class="form-group">
						...
						<div class="form-group">
							<label class="col-sm-2 control-label no-padding-right"
								for="inputRepairTime">维修时间</label>
							<div class="col-sm-4">
								<input type="text" id="inputRepairTime" name="inputRepairTime"  required />
							</div>

							<label class="col-sm-2 control-label no-padding-right"
								for="inputRepairType">维修类型</label>
							<div class="col-sm-4">
								<input type="text" id="inputRepairType" name="inputRepairType"  required />
							</div>
						</div>
						<div class="form-group">
							<label class="col-sm-2 control-label no-padding-right"
								for="inputRepairMemo">维修说明</label>
							<div class="col-sm-4">
								<input type="text" id="inputRepairMemo" name="inputRepairMemo"  required />
							</div>
						</div>
					</form>
				</div>
...
<script type="text/javascript">
	...
	function editObject(rowIndex) {
		...
		$("#hiddenRepairId").val(row["repairId"]);
		$("#inputUserProductLineId").val(row["userProductLineId"]);
		refreshInputUserEquipmentId();
		$("#inputUserEquipmentId").val(row["userEquipmentId"]);
		$("#inputRepairTime").val(row["repairTime"]);
		$("#inputRepairType").val(row["repairType"]);
		$("#inputRepairMemo").val(row["repairMemo"]);
	...
	function postObject() {
	    var obj=new Object();
		obj.repairId =$("#hiddenRepairId").val();
		
		obj.userEquipmentId=$("#inputUserEquipmentId").val();
		obj.repairTime=$("#inputRepairTime").val();
		obj.repairType=$("#inputRepairType").val();
		obj.repairMemo=$("#inputRepairMemo").val();
		
		var jsonStr = JSON.stringify(obj);
		//alert(jsonStr);
		var httpMethod="post";
		if($("#hiddenRepairId").val()<=0)
		  httpMethod="post";
		else
		  httpMethod="put";
		$.ajax({
	...
	function deleteObject() {
		...
		$.ajax({
			type : "delete",
			url : "../maintain/viRepairInfo/"+deleteId,
			async : false, //默认为true 异步   
			error : function() {
				alert('提交失败!');
			},
	...
	$('#inputRepairTime').datetimepicker({
		language : 'zh',
