...
<div class="row">
	<div class="col-xs-12">
		<form class="form-horizontal" role="form">
		    <input type="hidden" id="queryEquipmentId" name="queryEquipmentId"  value="${equipmentId}" />
		    <div class="form-group">
				...
				<label class="col-sm-2 control-label no-padding-right" for="queryParamType">参数类型</label>
				<div class="col-sm-4 clip-radio radio-primary">
					<input type="text" id="queryParamType" name="queryParamType" class="col-xs-10 col-sm-5" />
				</div>
			</div>
...
<div>
	<table id="dynamic-table"
		...
		<thead>
			<tr>
				<th class="center"><label class="pos-rel"> <input type="checkbox" class="ace" /> <span class="lbl"></span>
				</label></th>
				<th>设备名称</th>
				<th>参数名称</th>
				<th>参数类型</th>
			...
			<div class="modal-body">
				<div class="row">
					<form id="formInput" class="form-horizontal" role="form" >
						<input type="hidden" id="hiddenEquipmentParamId" name="hiddenEquipmentParamId" />
						<input type="hidden" id="inputEquipmentId" name="inputEquipmentId" value="${equipmentId}" />
						...
						<div class="form-group">
							...
							<div class="col-sm-4 clip-radio radio-primary">
								    <input type="radio" name="inputParamType" value="开关量"  />
								    <label>开关量</label>
								    <input type="radio" name="inputParamType" value="模拟量"  />
								    <label>模拟量</label>
							</div>
						</div>
						<div class="form-group">
...
<script type="text/javascript">
								...
								.DataTable(
										{
											...
											"ajax" : {
												url : '../basic/viEquipmentParam/',
												type : 'get',
												data : function(d) {
													d.queryEquipmentId=document.getElementById("queryEquipmentId").value;
                                                    d.queryParamName=document.getElementById("queryParamName").value;
                                                    d.queryParamType=document.getElementById("queryParamType").value;
												},
	...
	jQuery(function($) {
		...
		$("#btnReturn").on('click', function(e) {
			var url = "../basic/equipmentInfoPage";
			var title = "设备维护";
	...
	function editObject(rowIndex) {
		...
		$("#hiddenEquipmentParamId").val(row["equipmentParamId"]);
	...
	function postObject() {
	    var obj=new Object();
		obj.equipmentParamId =$("#hiddenEquipmentParamId").val();
		obj.equipmentId=$("#inputEquipmentId").val();
		obj.paramName=$("#inputParamName").val();
		//alert($("#inputParamTypeSwitch").checked);
		...
		//alert(jsonStr);
		var httpMethod="post";
		if($("#hiddenEquipmentParamId").val()<=0)
		  httpMethod="post";
		else
		  httpMethod="put";
		$.ajax({
			type : httpMethod,
			url : "../basic/equipmentParam/",
			data : jsonStr,
	...
	function deleteObject() {
		...
		$.ajax({
			type : "delete",
			url : "../basic/equipmentParam/"+deleteId,
			async : false, //默认为true 异步   
			error : function() {
				alert('提交失败!');
			},
