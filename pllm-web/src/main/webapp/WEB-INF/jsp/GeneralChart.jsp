...
<!-- 自定义的一些全局css -->
<link rel="stylesheet" href="assets/css/data_text3.css">
<!-- 自定义的正文表格部分的css -->
...
<!--headerr结束-->
<script>
	var userProductLineId;
	$(document).ready(function () {
	    $("#queryUserProductLineId").change(function () {
	    	var option = $("#queryUserProductLineId option:selected");  //获取选中的项
			userProductLineId = option.val();	
	    	//显示option选中的生产线一览图
			if(userProductLineId==null || userProductLineId=="" ){
	    		alert("请选型客户生产线")
	    		return;
	    	}
			var url = "../GeneralChart?userProductLineId=" + userProductLineId;
			//alert(url);
			var title = "生产线一览图";
			showPageContent(url, title);
	        //alert(userProductLineId);
	    });
	    var obj = document.getElementById("queryUserProductLineId");
	    var tag = false;
	    for(i=0;i<obj.length;i++){
	      if(obj[i].value == userProductLineId){
	    	    obj[i].selected = true;
	    	    tag = true;
	      }
	    }
...
<div class="data_content">
	<div class="col-sm-2" style="background-color: #F0F0F0; height: 35px;margin-left: 15px; margin-bottom: 25px;">
		<select style="background-color: #F0F0F0;border:0;" id="queryUserProductLineId" name="queryUserProductLineId" required>
			<option value=""></option>
